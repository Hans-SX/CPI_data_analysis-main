...
def save_refocused_image(Afile, Bfile, z, cyc, timer, save_path):
    ...
    if not os.path.exists(join(outpath, 'refVecs_exp_pos')):
        os.makedirs(join(outpath, 'refVecs_exp_pos'))
    ...
    np.save(join(outpath, 'refVecs_exp_pos', str(cyc + 1) + "_" + str(round(z, 3)) + "mm" + "_refVec.npy"), np.array(refVec, dtype=object), allow_pickle=True)

    # if (cyc+1) % 10 == 0:
    #     print("Iteration " + str(cyc+1) + " finished.")
...
# cyc = 0
save_path = join(outpath, 'refimages_exp_pos')
if not os.path.exists(save_path):
    os.makedirs(save_path)

# Parallel(n_jobs=-1, backend="loky")(
#     delayed(save_refocused_image)(Afile, Bfile, z, cyc, timer, save_path) for cyc, (Afile, Bfile, z) in enumerate(zip(armAfiles, armBfiles, avg_z))
#     )
Parallel(n_jobs=4, backend="loky")(
    delayed(save_refocused_image)(Afile, Bfile, z, cyc, timer, save_path) for Afile, Bfile, z, cyc in zip(armAfiles, armBfiles, avg_z, np.arange(total_iterations))
    )
timer.stop("Whole refocusing")
timer.savefile(join(outpath, "refocusing_times_exp_pos.txt"))