...
for Afile, Bfile, z in zip(armAfiles, armBfiles, REFOC):
    ...
    # np.save(save_path + "_refVec.npy", np.array(refVec, dtype=object), allow_pickle=True)
    sum_refVec = [np.sum(ref, axis=1) for ref in refVec]
    argmax_sum = [np.argmax(s) for s in sum_refVec]

    # gaussian_fits = Parallel(n_jobs=-1, backend="loky")(
    #     delayed(fit_gaussian)(np.arange(20), ref[s-10:s+10]) for ref, s in zip(sum_refVec, argmax_sum)
    # )
    gaussian_fits = Parallel(n_jobs=-1, backend="loky")(
    ...
    sigma = np.array([fit[2] if fit is not None else np.nan for fit in gaussian_fits])  # shape: (len(M_ratio),)
    
    print("sigma for z = {}: {}".format(z, sigma))
    plt_sigma(M_ratio, sigma, z, outpath, cyc+1)
...
#%%
np.save(join(outDir, "sigmas.npy"), np.array(sigmas))
