...
from utils import Calculating_G2, line, column_centroids
...
# for Afile, Bfile in zip(armAfiles, armBfiles):
    ...
    centroids_x = column_centroids(xaxb)
    centroids_y = column_centroids(yayb)

    range_x = np.where(np.sum(xaxb, axis=0) > 0.6 * np.max(np.sum(xaxb, axis=0)))[0]
    range_y = np.where(np.sum(yayb, axis=0) > 0.6 * np.max(np.sum(yayb, axis=0)))[0]
    # range_x = np.arange(30, 70)
    # range_y = np.arange(30, 70)
    x_popt, x_pcov = curve_fit(line, range_x, centroids_x[range_x])
    y_popt, y_pcov = curve_fit(line, range_y, centroids_y[range_y])
    ...
    ax1.scatter(range(centroids_x.shape[0]), centroids_x, color="blue")
    im2 = ax2.imshow(yayb, cmap="gray")
    ax2.plot(y_line, color="red")
    ax2.scatter(range(centroids_y.shape[0]), centroids_y, color="blue")
